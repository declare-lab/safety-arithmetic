...
class BaseProbInference:
    ...
    def get_icv(model, inputs, rank=1,model_type='base'):
        ...
        for demonstration_id in range(num_demonstration):
            if model_type == 'base' or model_type == 'sft':
                h = hidden_states[demonstration_id][0].flatten() - hidden_states[demonstration_id][1].flatten()
            # h = hidden_states[demonstration_id][0][:,1:4,:].flatten() - hidden_states[demonstration_id][1][:,1:4,:].flatten()
            elif model_type == 'edit':
                h = hidden_states[demonstration_id][0][:,4:7,:].flatten() - hidden_states[demonstration_id][1][:,4:7,:].flatten()
                num_layers = 3
        ...
        return direction.view(num_layers, hidden_dim)   #num_layers
        # return direction.view(3, hidden_dim)